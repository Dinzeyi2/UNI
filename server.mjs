...
const providers = {
  smartthings: { name: 'SmartThings', authorizationUrl: 'https://api.smartthings.com/oauth/authorize', scopes: ['r:devices:*', 'x:devices:*'], configured: Boolean(process.env.SMARTTHINGS_CLIENT_ID) },
  home_assistant: { name: 'Home Assistant', authorizationUrl: null, scopes: ['read', 'control'], configured: Boolean(process.env.HOME_ASSISTANT_URL && process.env.HOME_ASSISTANT_TOKEN) }
};
...
function compileBehavior(user, input) {
  const intent = String(input.intent || '').trim(); if (!intent) throw new Error('intent is required'); const lower = intent.toLowerCase(); const light = firstDevice(user, 'light.set_brightness'); const speaker = firstDevice(user, 'speaker.play'); const motion = firstDevice(user, 'sensor.motion_detected'); const thermostat = firstDevice(user, 'thermostat.set_temperature');
  const isNight = /sleep|night|arrival|arrive|bed/.test(lower); const isFocus = /focus|work|meeting/.test(lower); const actions = [];
  if (light) actions.push({ deviceId: light.deviceId, capability: 'light.set_brightness', parameters: { percent: isNight ? 10 : 70 }, reason: isNight ? 'Keep lighting low at night' : 'Create a focused workspace' });
  ...
  const behavior = { behaviorId: `beh_${randomBytes(12).toString('hex')}`, name: input.name || (isNight ? 'Night Navigation' : isFocus ? 'Deep Focus' : 'Custom Environment'), goal: intent, status: 'draft', version: 1, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), triggers: motion && isNight ? [{ type: 'sensor.motion_detected', deviceId: motion.deviceId }] : [{ type: 'manual.start' }], conditions: isNight ? [{ type: 'time.between', start: '23:00', end: '06:00' }] : [{ type: 'presence.home', value: true }], actions, safeguards: [{ rule: 'preserve_manual_override' }, { rule: 'never_unlock_doors' }], termination: [{ type: 'timeout', afterMinutes: isNight ? 10 : 120 }], requiredCapabilities: [...new Set(actions.map(action => action.capability))], versions: [] };
