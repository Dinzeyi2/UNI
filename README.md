...
The agent binds to `127.0.0.1:4180` by default. Its authenticated API provides `POST /discover`, `GET /inventory`, `POST /pair/hue`, and `POST /execute`. Hue pairing requires the user to press the physical bridge link button; its pairing credential is encrypted in the local state file and paired lights are imported as individually addressable capabilities. Built-in executable adapters support paired Hue Bridge lights, compatible Shelly switches, and Sonos playback/pause/volume through its local UPnP/SOAP API on port 1400. Unknown devices remain visible but non-executable instead of being probed with arbitrary packets. Expose the agent remotely only through a separately authenticated, encrypted tunnel; never port-forward it directly to the internet.

Local protocols are plugins rather than branches in the runtime. An adapter exports `id`, `name`, `match(device)`, `capabilities`, `execute(device, action, inventory)`, and optionally `pair(...)` and `verify(...)`. Add trusted ESM adapter module URLs to `NEXUS_ADAPTER_MODULES` as a comma-separated list; the agent validates each contract and refuses duplicate IDs. `GET /adapters` reports installed adapters. Execution accepts only an adapter's declared normalized capabilities and returns its post-command verification. Never load untrusted adapter code because plugins run with the local agent's operating-system permissions.

Matter and HomeKit use dedicated controller processes because commissioning and encrypted protocol sessions should not be reimplemented inside Nexus. Set `MATTER_BRIDGE_URL`/`MATTER_BRIDGE_TOKEN` or `HOMEKIT_BRIDGE_URL`/`HOMEKIT_BRIDGE_TOKEN` to enable their adapter plugins. Nexus accepts only loopback bridge URLs, sends pairing/execute/verify requests through a small authenticated JSON contract, and encrypts returned operational credentials in the local inventory. The controller sidecar must implement `POST /pair`, `POST /execute`, and `POST /verify`; it remains responsible for the real Matter or HAP library, radio/interface access, and setup-code validation.

For a deployable controller path without writing either secure protocol from scratch, connect the agent to a local Home Assistant installation using `LOCAL_HOME_ASSISTANT_URL` and a long-lived `LOCAL_HOME_ASSISTANT_TOKEN`. Pair Matter and HomeKit Controller accessories in that local controller once. Nexus then imports supported light, media-player, and climate entities, translates normalized actions into local service calls, and verifies the entity state after every command. This path stays on the LAN and also makes devices provided through Home Assistant's Matter, HomeKit Controller, Sonos, Zigbee, or Thread integrations available through one verified adapter.

`LOCAL_HOME_ASSISTANT_TOKEN` and `NEXUS_CLOUD_AGENT_TOKEN` are different secrets and are never interchangeable. Create the former in the Home Assistant user profile; create the latter with `POST /api/agents` in Nexus. If either token is printed or shared, revoke it at its issuing service and create a replacement before continuing.
...
This is a functional local creation path for Hue, Shelly, Sonos, and supported entities paired into a local Home Assistant controller, plus secure integration points for alternative Matter and HomeKit controller sidecars—not universal Wi-Fi control. Zigbee/Thread still require suitable coordinator/border-router hardware; broader context connectors, DHCP/MAC reconciliation, and spatial CSI sensing remain separate hardware or adapter work.
